...
Hooks.on("dnd5e.postUseActivity", async (activity, usageConfig, results) => {
  const targetUUID = "Compendium.UA2025-HorrorSubclasses.classes.Item.uahsSpiritsFromB";
...
async function executeSpiritsFromBeyond(actor) {
  try {
    const spiritsFromBeyond = await fromUuid("Compendium.UA2025-HorrorSubclasses.tables.RollTable.uahsTableSpirits");
    ...
    for (let i = 0; i < rollCount; i++) {
      ...
      for (const result of draw.results) {
        if (result.description) {
          result.description = await TextEditor.implementation.enrichHTML(result.description, {
            rollData: rollData,
